...
# Creating folders for contour (if not already there)
...
for error_type in range(6):
    for i in range(1, 13):
        for j in range(1, 21):
            img1_path = f"Output_data/Subtraction/{errors[error_type]}/{i:02d}_{errors[error_type]}_{j:02d}.jpg"
            img2_path = f"Output_data/BW_images/{errors[error_type]}/{i:02d}_{errors[error_type]}_{j:02d}.jpg"
            ...
            contours1, hierarchy1 = cv2.findContours(img1, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
            ...
            for a, cnt in enumerate(contours1):
                ...
                cv2.imwrite(
                    f"Output_data/ROI/{errors[error_type]}/{i:02d}_{errors[error_type]}_{j:02d}/"
                    f"{i:02d}_{errors[error_type]}_{j:02d}_roi_{a}.jpg", roi
                )
