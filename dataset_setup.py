...
if not defect_folders:
    raise ValueError("No defect subfolders found inside template!")
...
for defect in defect_folders:
    ...
    if not template_images or not test_images:
        print(f" Skipping {defect}: missing images.")
        continue
    ...
    for idx, temp_name in enumerate(template_images):
        ...
        if template is None or test is None:
            print(f" Error reading pair {temp_name} / {test_images[idx]}")
            continue
...
print(f" Output masks stored in: {output_root}")
